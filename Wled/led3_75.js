...
const IP_EFFECT_GROUPS = {
  '192.168.0.118': {
    effects: [EFFECTS.LIGHTNING, EFFECTS.DANCING_SHADOW],
    type: 'random'
  },
  '192.168.0.174': {
    effects: [EFFECTS.CRAZY_BEES],
    type: 'fixed'
  },
  '192.168.0.151': {
    effects: [EFFECTS.LISS, EFFECTS.RIPPLE],
    type: 'random'
  },
  '192.168.0.142': {
    effects: [EFFECTS.RAIN, EFFECTS.DRIP],
    type: 'random'
  }
};
...
function selectEffect(ip) {
  const effectGroup = IP_EFFECT_GROUPS[ip];
  if (effectGroup.type === 'fixed') {
    return effectGroup.effects[0];
  } else if (effectGroup.type === 'random') {
    return effectGroup.effects[Math.floor(Math.random() * effectGroup.effects.length)];
  }
}
...
function prepareCommands(normalizedData) {
  ...
  return ESP32_IPS.map((ip, index) => {
    ...
    const selectedEffect = selectEffect(ip);
